...
resource "azurerm_virtual_machine" "stage-tomcat" {
  name                  = "stage-tomcat-vm"
  location              = "${azurerm_resource_group.stage-tomcat.location}"
  availability_set_id   = "${azurerm_availability_set.stage-tomcat-as.id}"
  resource_group_name   = "${azurerm_resource_group.stage-tomcat.name}"
  network_interface_ids = ["${azurerm_network_interface.stage-tomcat.id}"]
  ...
  # Optional data disks
  storage_data_disk {
    name              = "stage-tomcat-datadisk-001"
    managed_disk_type = "Premium_LRS"
    create_option     = "Empty"
    lun               = 0
    disk_size_gb      = "1023"
  ...
  storage_data_disk {
    name            = "${azurerm_managed_disk.stage-tomcat.name}"
    managed_disk_id = "${azurerm_managed_disk.stage-tomcat.id}"
